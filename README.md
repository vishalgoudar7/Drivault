...
*   enter `adb logcat --pid=$(adb shell pidof -s 'com.drivault') > logcatOutput.txt` to save the output to this file
...
*   enter `adb shell pidof -s 'com.drivault'` and use the output as `<processID>` in the following command:
...
*   enter `logcat -d --pid $(pidof -s com.drivault) -f /sdcard/logcatOutput.txt`
